console.log("++++++++++++++++ IAAI extension is starting... ++++++++++++++++++++++++");

var find_vin_code = setInterval(function() {
    if ($("#kbb_price_div").length == 0 && $("#VIN_vehicleStats1") && $("#ProductDetailsVM")) {
        myMain();
    }
}, 1000);
...
async function myMain (evt) {
    ...
    console.log("++++++++++++++++ IAAI extension is working... ++++++++++++++++++++++++");

/***********************************    get info    *********************************************/
    var allInfoObj = JSON.parse($("#ProductDetailsVM").text());
    cor_vin = $("#VIN_vehicleStats1").text().trim();
    await getDetailInfo(allInfoObj);
    cor_zipcode = "97220";
    if(!cor_vin && !cor_mileage && !cor_year) return;

/***********************************    start show price    *********************************************/
    var lot_detail = $(".data-container > .row > div:first");
    var priceDiv = document.createElement('div');
    priceDiv.setAttribute('id', 'kbb_price_div');
    priceDiv.setAttribute('class', 'kbb_price_div_class tile tile--data');

    lot_detail[0].insertBefore(priceDiv, lot_detail[0].firstChild);

    var historyDiv = document.createElement('div');
    historyDiv.setAttribute('id', 'history_div');
    historyDiv.setAttribute('class', 'history_div_class tile tile--data');

    $(historyDiv).insertAfter(priceDiv);

    historyDiv.innerHTML = `<div class="tile-header">
            <h2 class="data-title">Trade History</h2>
        </div>
        <div id="trade_history_div" class="tile-body">
    ...
    var salvage_detail = $(".data-container > .row > div:nth-child(2)");
    var salvageDiv = document.createElement("div");
    salvageDiv.setAttribute("id", "salvage_price_div");
    salvageDiv.setAttribute("class", "salvage_price_div_class tile tile--data");

    salvage_detail[0].insertBefore(salvageDiv, salvage_detail[0].firstChild);

    var carpartDiv = document.createElement("div");
    carpartDiv.setAttribute("id", "carpart_price_div");
    carpartDiv.setAttribute("class", "carpart_price_div_class tile tile--data");

    $(carpartDiv).insertAfter(salvage_detail[0].firstChild);

    carpartDiv.innerHTML = `<div class="tile-header">
            <h2 id="carparts_list_panel" class="data-title">Parts
                <span class="bold right" id="total_carpart_price">$0</span>
                <span class="bold right" id="labor_total"> Labor Total : </span>
            </h2>
        </div>
        <div class="tile-body">
            <div id="carpart_price_list"></div>
        </div>`;

    var feeDiv = document.createElement("div");
    feeDiv.setAttribute("id", "fee_price_div");
    feeDiv.setAttribute("class", "fee_price_div_class tile tile--data");

    $(feeDiv).insertAfter(salvage_detail[0].firstChild);
    ...
    if (car_values && car_values.data && car_values.data.ymmt) {
        ...
        priceDiv_content =  '<div class="tile-header">' +
                    '<h2 id="kbb_price_panel" class="data-title">KBB <select id="price_type"><option value="tradein" selected>Trade IN</option><option value="privateparty">Private Party</option></select> Condition' +
        ...
        salvageDiv_content = `<div class="tile-header">
            <h2 id="salvage_price_panel" class="data-title">KBB Price
        ...
        priceDiv_content =  '<div class="tile-header">' +
                    '<h2 class="data-title">KBB Clean Trade-In<span class="bold right one-click-select" id="lot-price">undefined</span></h2>' +
                '</div>' +
    ...
    feeDiv.innerHTML = `<div class="tile-header">
            <h2 id="fee_price_panel" class="data-title">Bid: 
...
/***********************************    start select sub item    *********************************************/
    ...
    var targetDiv = document.getElementsByClassName("vehicle-image");
    ...
    loading_img.setAttribute("src", chrome.runtime.getURL('images/preloader.svg'));
    car_map_loading.append(loading_img);
    car_map_div.append(car_map_loading);
    $(car_map_loading).hide();

    $(car_map_div).insertAfter(targetDiv);
...
function getDetailInfo(object) {
    return new Promise(resolve => {
        if(object && object.VehicleDetailsViewModel) {
            cond = object.VehicleDetailsViewModel.ConditionInfo;
            for( var item of cond) {
                if(item && item.Name && item.Name == "Odometer") {
                    var _txt = item.DisplayValues[0].Text;
                    cor_mileage = _txt.trim().slice(0, -3).replaceAll(',', '');
                    break;
                }
            }
            cor_year = object.VehicleDetailsViewModel.Year;
            current_bid_price = parseFloat(object.VehicleDetailsViewModel.HighBidAmount.slice(1).replaceAll(',', '')).toFixed(2);
        } else if(object && object.inventoryView) {
            cor_vin = object.inventoryView.attributes.VIN;
            cor_mileage = object.inventoryView.attributes.ODOValue;
            cor_year = object.inventoryView.attributes.Year;
            current_bid_price = parseFloat(object.inventoryView.highBidAmount.slice(1).replaceAll(',', '')).toFixed(2);
        }
        resolve(true);
    });
}
...
function showCarParts(result) {
    if($("#carparts-table").length < 1) {
        ...
        carpartsDiv.setAttribute("class", "carparts_div_class tile tile--data");

        $(carpartsDiv).insertAfter($("#waypoint-trigger"));
    }
    var car_content = '';
    if(result) {
        car_content = `<div class="tile-header"><h2 class="data-title">${carpart_name}</h2></div>
            <div class="carparts-list tile-body row">`;
...
function getEstimatedDeliveryDates(send_data){
    return new Promise( (resolve, reject) => {
        chrome.runtime.sendMessage({ from: "sendIAAICarpartsDeliveryDates", data: send_data }, (response3) => {
...
function getCarPartList(data, area_key) {
    ...
    chrome.runtime.sendMessage({ from: "sendIAAICopartReq", data: builFormData(data) }, (response) => {
        showTableforCarPart(response.data, area_key);
    });
    console.log("22222222222222222222222222222222222222222222222222222222");
    chrome.runtime.sendMessage({ from: "sendIAAICarpartsDetail", data: data_3 }, (response1) => {
        var total_result = {};
        ...
        if(result_1.data.products && result_1.data.products.productIds) {
            ...
            chrome.runtime.sendMessage({ from: "sendIAAICarpartsBatch", data: {items: result_1.data.products.productIds} }, async (response2) => {
    ...
    var car_map = `<div id="car_map">
                    ...
                    <div id="car_part">
                        ...
                        <a href="javascript:void(0);" title="Roll Over Car" id="sub-img-group">
                            ...
                            <img id="sub-itme-img" src="../images/hood.jpg" style="display: none;">
                        </a>
                        <map name="carpartmap">
                        </map>
                    </div>`;
...
$('body').on('click', '#car_part_table tr:not(:first):not(:last)', function(){
    ...
    if($(".carpart_price_item[key=" + area_key + "]").length) {
        var _current_price = Number($(".carpart_price_item[key=" + area_key + "] .carpart-price").text().replace(/[^0-9\.]+/g,""));
        $(".carpart_price_item[key=" + area_key + "] .carpart-price").text(`$${_price}`);
        $('#total_carpart_price').text(`$${(_total + _price - _current_price).toFixed(2)}`)
    } else {
        $("#carpart_price_list").append(`<p class="carpart_price_item" key="${area_key}"><span>${carpart_name}</span><span class="carpart-price">$${_price}</span></p>`)
